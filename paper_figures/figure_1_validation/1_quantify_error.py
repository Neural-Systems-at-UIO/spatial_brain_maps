...
def create_group_atlas_overlay(aligns, markers, human_names, annot, raw_outline3d, template, H, W, section_nr, brain_id):
    """
    ...
    if not mats:
        ...
        return
    nl_path = mats[0].replace("_SyN_affineTransfo.mat", "_SyN_nonLinearDf.nii.gz").replace("affine", "nonlin")
    if os.path.exists(nl_path):
        nl_im = nib.load(nl_path).get_fdata()
        ny, nx = nl_im.shape[0], nl_im.shape[1]
    else:
        print(f"Nonlinear deformation field not found for brain {brain_id} section {sec4}")
        return
...
for brain in brain_ids:
    ...
    for sec in section_nrs:
        ...
        create_group_atlas_overlay(aligns, markers, human_names, annot, raw_outline3d, template, H, W, sec, brain)
