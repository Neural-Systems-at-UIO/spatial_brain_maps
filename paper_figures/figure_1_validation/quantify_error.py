...
AFFINE_FOLDER = "/home/harryc/github/spatial_brain_maps/paper_figures/validation_study/raters/pipeline_registrations/affine/"
...
def calculate_affine(source_points, destination_points):
    ...
    destination_points = np.hstack((destination_points, np.ones((destination_points.shape[0], 1))))
    affine_matrix, _, _, _ = np.linalg.lstsq(source_points, destination_points, rcond=None)
...
def read_ants_affine(affine_path):
    ...
    transformed_points = np.array([ants_transform.apply_to_point(p) for p in base_points])
...
def find_plane_equation(plane_params):
    a, b, c = (np.array(plane_params[:3], float),
...
def get_angle(plane_params, direction):
    ...
    if direction == "ML":
        a = plane_coords[0:2]
        linear_point = (((plane_coords[0] - 100) * normal[0]) +
                        ((plane_coords[2]) * normal[2])) + k
        depth = -(linear_point / normal[1])
        b = np.array((plane_coords[0] - 100, depth))
        c = b + [100, 0]
    if direction == "DV":
        a = plane_coords[1:3]
        linear_point = (((plane_coords[0]) * normal[0]) +
                        ((plane_coords[2] - 100) * normal[2])) + k
        depth = -(linear_point / normal[1])
...
def load_alignments(section_nr, human_files, ds_files, aba_files, ds_y_shift=24, ds_basic_files=None):
    aligns = {}
    markers = {}
    # 1) load humans
...
def compute_error(test_name, test_align, ref_aligns, annot, h, w, section_nr=None, brain_id=None, test_markers=None, ref_markers=None):
    ...
    aff_pat = os.path.join(AFFINE_FOLDER, brain_id, "*", f"*_s{sec4}_SyN_affineTransfo.mat")
    mats = glob(aff_pat)
    nl_path = mats[0].replace("_SyN_affineTransfo.mat", "_SyN_nonLinearDf.nii.gz") \
    ...
    if ref_markers is not None:
        ...
        for ref_m in ref_markers:
            ...
            if ref_m is not None:
                ...
                tempx_copy, tempy_copy = transform_vec(triangulation, tempx_copy, tempy_copy)
    ...
    im_path = glob(f'/home/harryc/github/spatial_brain_maps/paper_figures/validation_study/section_images/{brain_id}/thumbnails/*_s{sec4}.jpg')[0]
    image = cv2.imread(im_path)
    os.makedirs(f'template_test/{brain_id}/{test_name}', exist_ok=True)
    image  = cv2.resize(image, (at.shape[1], at.shape[0]))
    image[at != 0] = [0, 0, 255]
    cv2.imwrite(f'template_test/{brain_id}/{test_name}/{sec4}.jpg', image)
    if test_name == 'Our Pipeline':
        #only run this during our pipeline as thats when all humans are reference
        gtx[gtx < 0] = 0
        gty[gty < 0] = 0
        gtz[gtz < 0] = 0
        ...
        im_path = glob(f'/home/harryc/github/spatial_brain_maps/paper_figures/validation_study/section_images/{brain_id}/thumbnails/*_s{sec4}.jpg')[0]
        image = cv2.imread(im_path)
        os.makedirs(f'template_test/{brain_id}/group_average', exist_ok=True)
        image  = cv2.resize(image, (at.shape[1], at.shape[0]))
        image[at != 0] = [0, 0, 255]
        cv2.imwrite(f'template_test/{brain_id}/group_average/{sec4}.jpg', image)

    return np.mean(np.linalg.norm(diffs, axis=1))


def leave_one_out_humans(aligns, human_names, annot, h, w, section_nr, brain_id, markers):
    """
    ...
    for name in human_names:
        ...
        errs[name] = compute_error(name, aligns[name], refs, annot, h, w,section_nr,brain_id, markers[name], ref_m)
    return errs

def test_vs_all_humans(aligns, test_name, human_names, annot, h, w, section_nr=None, brain_id=None, markers=None):
    """
    ...
    for excluded_name in human_names:
        ...
        ref_m = [markers[n] for n in human_names if n != name]
        e = compute_error(test_name, aligns[test_name], refs, annot, h, w,
                         section_nr=section_nr, brain_id=brain_id, test_markers=None, ref_markers=ref_m)
        errors.append(e)
...
expert_files = {
    "Expert 1": glob("raters/experts/ingvild/*.json"),
    "Expert 2": glob("raters/experts/sharon/*.json"),
    "Expert 3": glob("raters/experts/simon/*.json"),
}
novice_files = {
    "Novice 1": glob("raters/novices/signy/*.json"),
    "Novice 2": glob("raters/novices/sophia/*.json"),
    "Novice 3": glob("raters/novices/archana/*.json"),
}
...
ds_files = glob("raters/pipeline_registrations/ds_human_affine/*.json")
aba_files = glob("raters/ABA/*.json")
...
for brain in brain_ids:
    ...
    hf_brain = {
        name: [p for p in human_files[name]
                if os.path.splitext(os.path.basename(p))[0] == brain]
        for name in human_names
    }
    ds_brain = [p for p in ds_files if os.path.splitext(os.path.basename(p))[0] == brain]
    aba_brain = [p for p in aba_files if os.path.splitext(os.path.basename(p))[0] == brain]
    ...
    for sec in section_nrs:
        ...
        human_errs = leave_one_out_humans(aligns, human_names, annot, H, W, sec, brain, markers)
        ...
        for method_key, method_name in methods:
            if method_name in aligns:
                ...
                mean_err = test_vs_all_humans(aligns, method_name, human_names, annot, H, W, sec, brain, markers)
