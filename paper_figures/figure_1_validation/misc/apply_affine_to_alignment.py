...
raw_outline3d = find_boundaries(annot, mode="inner")
...
temp_image, pad_top, pad_bottom, pad_left, pad_right = apply_affine_to_image(
    temp_image, np.eye(3), (align_height, align_width), transform_constant=0
)
...
temp_image = cv2.resize(image, (int(image.shape[1] // 2.5), int(image.shape[0] // 2.5)))
temp_image, pad_top, pad_bottom, pad_left, pad_right = apply_affine_to_image(
    temp_image, affine_matrix, (align_height, align_width), transform_constant=0
)
orig_pts = np.array(
    [
        [pad_top, pad_left],
        [pad_top, temp_image.shape[1] - pad_right],
        [temp_image.shape[0] - pad_bottom, pad_left],
    ]
)
...
temp_image = cv2.resize(
    image,
    (
        int(np.linalg.norm(affed_align[3:6])) + 1,
        int(np.linalg.norm(affed_align[6:])) + 1,
    ),
)
...
def debug_visualization(image, outline, title, show_padding=True):
    ...
    try:
        ...
        if image.shape[:2] != outline.shape:
            print(
                f"Warning: Shape mismatch - Image: {image.shape}, Outline: {outline.shape}"
            )
            # Resize outline to match image if possible
            outline = cv2.resize(
                outline.astype(float), (image.shape[1], image.shape[0])
            )
        ...
        outline_mask = (
            (outline != 0)
            & (outline.shape[0] == overlay.shape[0])
            & (outline.shape[1] == overlay.shape[1])
        )
...
warped_outline = affine_transform(
    outline.astype(float), inv_affine, output_shape=(align_h, align_w), order=0
)
