...
from paper_figures.validation_study.affine_utliities import (
    update_alignment_with_ants_affine,
    read_ants_affine,
)
import numpy as np
import cv2
...
for brain in tqdm(brains):
    ...
    for index, section in enumerate(data["slices"][:]):
        ...
        image_path = glob(
            f"{root_dir}/section_images/{b}/thumbnails/*s{str(nr).zfill(4)}.jpg"
        )
        affine_path = glob(
            f"{root_dir}/raters/pipeline_registrations/affine/{b}/*/*_s{str(nr).zfill(4)}_SyN_affineTransfo.mat"
        )
        if image_path:
        ...
        final_alignment = update_alignment_with_ants_affine(
            alignment, MATRIX, o_image.shape
        )
        section["anchoring"] = list(final_alignment)
        new_slices.append(section)
    out["slices"] = new_slices
    with open(
        f"{root_dir}/raters/pipeline_registrations/ds_human_affine/{b}.json", "w"
    ) as file:
        json.dump(out, file)
