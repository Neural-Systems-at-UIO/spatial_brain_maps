...
for fn in files:
    try:
        data = PyNutil.io.read_and_write.load_quint_json(fn, propagate_missing_values=False)
        slices = data['slices']
        ux = [s['anchoring'][3] for s in slices]
    except Exception:
        data = PyNutil.io.read_and_write.load_quint_json(fn, propagate_missing_values=True)
        slices = data['slices']
        ux = [s['anchoring'][3] for s in slices]
