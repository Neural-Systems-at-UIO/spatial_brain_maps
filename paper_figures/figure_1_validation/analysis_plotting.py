...
def load_and_prepare_data():
    df = pd.read_csv('consolidated_registration_results.csv')
    
    # Melt the dataframe to convert to long format
    raters = ['Expert 1', 'Expert 2', 'Expert 3', 'Novice 1', 'Novice 2', 'Novice 3',
              'Our Pipeline', 'ABA']
    for rater in raters:
    ...
    human_stats = df[['brain_id', 'section_nr', 'human_ml_avg', 'human_ml_std', 
                      'human_dv_avg', 'human_dv_std']]
...
def plot_angle_comparisons(df):
    ...
    angle_data = df.groupby(['brain_id', 'rater', 'group']).agg({
        'ml_angle': 'mean',
        'dv_angle': 'mean',
        'human_ml_avg': 'first',
        'human_ml_std': 'first',
    ...
    # For Our Pipeline
    y_true_pipeline = np.concatenate([pipeline_data['human_ml_avg'], pipeline_data['human_dv_avg']])
    y_pred_pipeline = np.concatenate([pipeline_data['ml_angle'], pipeline_data['dv_angle']])
    ...
    pipeline_data = methods[methods['group'] == 'Our Pipeline']
    axes[0,0].errorbar(pipeline_data['human_ml_avg'],
                      pipeline_data['ml_angle'],
                      xerr=pipeline_data['human_ml_std'],
                      fmt='o', alpha=0.7, markerfacecolor='white', 
                      markeredgecolor='black', ecolor='gray', capsize=3)
    ...
    # ABA ML Angle
    aba_data = methods[methods['group'] == 'ABA']
    axes[0,1].errorbar(aba_data['human_ml_avg'],
                      aba_data['ml_angle'],
    ...
    axes[1,0].errorbar(pipeline_data['human_dv_avg'],
    ...
    # ABA DV Angle
    axes[1,1].errorbar(aba_data['human_dv_avg'],
                      aba_data['dv_angle'],
...
def plot_combined_angle_comparisons(df):
    ...
    for idx, method in enumerate(method_names):
        ...
        # ML
        axes[idx].errorbar(
            method_data['human_ml_avg'], method_data['ml_angle'],
            xerr=method_data['human_ml_std'], fmt='o', color=colors['ML'],
            alpha=0.7, markerfacecolor='white', markeredgecolor=colors['ML'],
            ecolor=colors['ML'], capsize=3, label='ML'
        )
        # DV
        axes[idx].errorbar(
            method_data['human_dv_avg'], method_data['dv_angle'],
            xerr=method_data['human_dv_std'], fmt='s', color=colors['DV'],
            alpha=0.7, markerfacecolor='white', markeredgecolor=colors['DV'],
            ecolor=colors['DV'], capsize=3, label='DV'
        )
        # 1:1 lines for both
        min_val = min(
            method_data['human_ml_avg'].min(), method_data['ml_angle'].min(),
            method_data['human_dv_avg'].min(), method_data['dv_angle'].min()
        )
        max_val = max(
            method_data['human_ml_avg'].max(), method_data['ml_angle'].max(),
            method_data['human_dv_avg'].max(), method_data['dv_angle'].max()
        )
        axes[idx].plot([min_val-1, max_val+1], [min_val-1, max_val+1], 'k--', alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig('combined_angle_comparisons.png')
    plt.savefig('combined_angle_comparisons.pdf')
    plt.show()
...
def plot_error_distributions(df):
    ...
    sns.swarmplot(x='rater', y='error', hue='group', data=plot_data, size=3, legend=False, ax=ax)
    ...
    for group, positions in group_positions.items():
        ...
        ax.hlines(mean_val, x_min, x_max, 
    ...
    anova_data = pd.DataFrame({
        'error': pd.concat([expert_errors, novice_errors, pipeline_errors, aba_errors]),
        'group': ['Expert']*len(expert_errors) + ['Novice']*len(novice_errors) + 
                ['Our Pipeline']*len(pipeline_errors) + ['ABA']*len(aba_errors)
    })
    ...
    comparisons = [
        ...
        ("Our Pipeline vs Expert", "Our Pipeline", "Expert", 6, (0, 2)),  # Pipeline to Expert group span
        ("Novice vs ABA", "Novice", "ABA", (3, 5), 7),      # Novice group span to ABA
        ("Our Pipeline vs Novice", "Our Pipeline", "Novice", 6, (3, 5)),  # Pipeline to Novice group span
        ("Our Pipeline vs ABA", "Our Pipeline", "ABA", 6, 7)  # Pipeline to ABA (both single points)
    ]
    ...
    for i, (name, group1, group2, pos1, pos2) in enumerate(comparisons):
        ...
        tukey_row = tukey_data[(tukey_data['group1'] == group1) & (tukey_data['group2'] == group2)]
        if len(tukey_row) == 0:
            tukey_row = tukey_data[(tukey_data['group1'] == group2) & (tukey_data['group2'] == group1)]
        ...
        if len(tukey_row) > 0:
            ...
            stem_height = bar_height * 0.4  # Small vertical stems connecting to brackets
            ...
            if isinstance(pos1, tuple):
                ...
                if group1 not in prev_groups:
                    ax.plot([x1_mid, x1_mid], [y_pos, y_pos - stem_height], 'k-', linewidth=1.2)
                    ...
                    ax.plot([x1_start, x1_end], [bracket_y, bracket_y], color=colour, linewidth=3)
                    ...
                    ax.plot([x1_mid, x1_mid], [y_pos, y_pos - (stem_height * 6)], 'k-', linewidth=1.2)


            else:
                ax.plot([x1_mid, x1_mid], [y_pos - bracket_height/2, y_pos + bracket_height/2], 'k-', linewidth=1.2)
            ...
            if isinstance(pos2, tuple):
                ...
                if group2 not in prev_groups:
                    ax.plot([x2_mid, x2_mid], [y_pos, y_pos - stem_height], 'k-', linewidth=1.2)
                    ...
                    ax.plot([x2_mid, x2_mid], [y_pos, y_pos - (stem_height *6 )], 'k-', linewidth=1.2)

            else:
                ax.plot([x2_mid, x2_mid], [y_pos - bracket_height/2, y_pos + bracket_height/2], 'k-', linewidth=1.2)
            ...
            ax.text(overall_mid, y_pos + bar_height*0.3, sig_with_p, 
    ...
    plt.savefig('error_distributions.pdf', dpi=300, bbox_inches='tight')
    plt.show()
...
def perform_statistical_tests(df):
    ...
    groups = [anova_data[anova_data['group'] == g]['error'] for g in ['Expert', 'Novice', 'Our Pipeline', 'ABA']]
        ...
        for i in range(len(groups)):
            for j in range(i+1, len(groups)):  # Only upper triangle (i < j)
                ...
                if len(tukey_row) > 0:
                    ...
                    comparison_text = f"{group1} vs {group2}\nMean diff: {diff:.1f}\n{sig_str}"
                    
                    # Add to plot - only in upper triangle
                    axes[i, j].axis('on')
                    axes[i, j].text(0.5, 0.5, comparison_text, 
                    ...
                    if reject:
                        ...
                        axes[i, j].set_facecolor('#eeffee')  # Light green for non-significant
        
        plt.suptitle('Statistical Comparisons of Registration Error Between Groups\n(ANOVA with Tukey\'s HSD)', fontsize=16)
