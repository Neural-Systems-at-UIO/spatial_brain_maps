...
with ThreadPoolExecutor() as executor:
    ...
    future_to_idx = {
        executor.submit(process_file, f): idx for idx, f in enumerate(files)
    }

    # collect results as they complete, but place them at the right index
    for fut in tqdm(
        as_completed(future_to_idx), total=len(files), desc="Processing files"
    ):
...
for batch in tqdm(
    gen_batches(n_samples, batch_size),
...
for i, batch in enumerate(
    tqdm(
        gen_batches(voxels.shape[0], batch_size),
        total=voxels.shape[0] // batch_size,
        desc="PCA transform",
    )
):
